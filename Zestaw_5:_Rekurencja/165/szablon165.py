# ====================================================================================================>
# Zadanie 165
# Dana jest tablica T[N] zawierająca liczby naturalne. Proszę napisać funkcję, która odpo-
# wiadanapytanie,czyspośród(niekonieczniewszystkich)elementówtablicymożnautworzyćdwapodzbiory
# ojednakowejsumieelementów,takabysumamocyobupodzbiorówwynosiłak.Dofunkcjinależyprzekazać
# wyłącznie tablicę T oraz liczbę naturalną k, funkcja powinna zwrócić wartość typu bool.
# ====================================================================================================>


def Zadanie_165(): ...


if __name__ == "__main__":
    from testy165 import Testy165

    Zadanie_165()
    # Testy165.StworzTesty([Zadanie_165])
