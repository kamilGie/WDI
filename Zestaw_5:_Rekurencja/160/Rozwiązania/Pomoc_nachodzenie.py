...
pygame.display.set_caption("Wizualizacja Kwadratów")
...
def rysuj_linie_i_napis(punkty1, punkty2, index1, index2, tekst, offset_x=0, offset_y=0):
...
def rysuj_relacje_nachodzenia(punkty_kwadrat1, punkty_kwadrat2):
    ...
    if punkty_kwadrat1[2][0] <= punkty_kwadrat2[0][0]:
        rysuj_linie_i_napis(punkty_kwadrat1, punkty_kwadrat2, 2, 0, "lewo", offset_x=-20)
    if punkty_kwadrat1[0][0] >= punkty_kwadrat2[2][0]:
        rysuj_linie_i_napis(punkty_kwadrat1, punkty_kwadrat2, 0, 2, "prawo", offset_x=20)
    if punkty_kwadrat1[3][1] <= punkty_kwadrat2[1][1]:
        rysuj_linie_i_napis(punkty_kwadrat1, punkty_kwadrat2, 3, 1, "góra", offset_y=-30)
...
while dziala:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            for i, (x, y) in enumerate(kwadraty):
                if x <= event.pos[0] <= x + bok_kwadratu and y <= event.pos[1] <= y + bok_kwadratu:
    ...
    if len(kwadraty) == 2:
        ...
        punkty_kwadrat1 = [(x1, y1), (x1 + bok_kwadratu, y1), (x1 + bok_kwadratu, y1 + bok_kwadratu), (x1, y1 + bok_kwadratu)]
        punkty_kwadrat2 = [(x2, y2), (x2 + bok_kwadratu, y2), (x2 + bok_kwadratu, y2 + bok_kwadratu), (x2, y2 + bok_kwadratu)]
